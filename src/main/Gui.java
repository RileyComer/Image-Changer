...
public class Gui extends JFrame{ 
	...
	private class HandlerClass implements MouseListener,MouseMotionListener{
		...
		public void mouseReleased(MouseEvent e) {
			...
			int startX, startY;
			if(mode[0].equals("Menu")) {
				if((getWidth()/(3))<getHeight()/8) {
					size=getWidth()/(3);
				}else {
					size=getHeight()/(8);
				}
				startX=(int)((getWidth()/2.0)-((size*3)/2.0));
				startY=(int)((getHeight()/2.0)-((size*8)/2.0));
				
				if((x>startX&&x<(startX+(size*3)))&&(y>startY+(size*2)&&y<(startY+(size*8)))) {
					if(y<startY+(size*3)) {
						window.setMode("Mode1");
					}else if(y<startY+(size*4)) {
						window.setMode("Mode2");
					}else if(y<startY+(size*5)) {
						window.setMode("Mode3");
					}else if(y<startY+(size*6)) {
						window.setMode("Mode4");
					}else if(y<startY+(size*7)) {
						window.setMode("Mode5");
					}else if(y<startY+(size*8)) {
						System.exit(0);
					}
				}
			}
